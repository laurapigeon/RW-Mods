...
sealed class InchwormAI : ArtificialIntelligence, IUseARelationshipTracker
{
    enum Behavior
    {
        Idle,
    ...
    sealed class InchwormTrackedState : RelationshipTracker.TrackedCreatureState
    {
    ...
    public InchwormAI(AbstractCreature acrit, Inchworm bug) : base(acrit, acrit.world)
    {
        this.bug = bug;
        bug.AI = this;
    ...
    {
        return new InchwormTrackedState();
    }
    ...
    {
        if (dRelation.state is not InchwormTrackedState state) return default;
    ...
    public override void Update()
    {
        ...
        {
            if (behavior is not Behavior.Idle)
            {
                behaviorCounter = 0;
                behavior = Behavior.Idle;
            }
